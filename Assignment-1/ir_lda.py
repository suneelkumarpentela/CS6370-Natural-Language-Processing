...
# docs_txt = json.load(open("cranfield/" + "stopword_removed_docs.txt", 'r'))[:]
# queries_txt = json.load(open("cranfield/" + "stopword_removed_queries.txt", 'r'))[:]
...
for idx in range(len(queries)):
    queries[idx] = queries[idx].lower()  # Convert to lowercase.
    queries[idx] = tokenizer.tokenize(queries[idx])  # Split into words.

# Remove numbers, but not words that contain numbers.
queries = [[token for token in query if not token.isnumeric()] for query in queries]

# Remove words that are only one character.
queries = [[token for token in query if len(token) > 1] for query in queries]
...
queries = [[lemmatizer.lemmatize(token) for token in query] for query in queries]
...
query_corpus = [q_dict.doc2bow(query) for query in queries]
